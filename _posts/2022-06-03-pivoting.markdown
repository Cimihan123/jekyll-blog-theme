...
So how can we access **"C"** from **"A"**? Well, that's direcly not possbile so we use the concept of pivoting. **"A"** can use **"B"** as the foothold to get access to **"C"**.
...
*Victim1* and *Victim2* are our targets.They are on  same subnet *172.16.101.0/24*  while *Attacker* is on different subnet *172.16.100.0/24*.
...
Notice *helper* container is built locally. This makes easy to customize the base image.
...
It is the plotting tool for system statistics. There is an issue in this tool. We will discuss about this later on. For now just assume there is a vulnerability in this tool.

![ victim1 image ><](/assets/img/blog4-pivoting/6-victim1-image.png)
<p align="center" style="color:white;">Fig: victim1.Dockerfile</p>

For *victim2* there is no need to manipulate the image locally. There is already a vulnerable wordpress in docker hub. We just pull it from there. Below image show the structure to configure the wordpress as we also need a database server. They are in same sub-network so they can communicate with each other.
...
As far now we have brief overview of our lab environment. Now we run the docker compose files and check them.
...
There are total of 5 containers running.Once *Attacker*, *helper*, *victim1* and *victim2* (wordpress & mysql) are running.
...
<p align="center" style="color:white;">Fig:Ping check from attacker </p>

![ pinging from attacher to helper image ><](/assets/img/blog4-pivoting/9-attacker-ping-helper.png)
<p align="center" style="color:white;">Fig:Ping check from attacker </p>

I guess you can figure it out the connections among containers from the below picture.
...
So there is port 22 (ssh) open.We have hydra installed in the attacker machine and there is a rockyou.txt file.
...
*ifconfig* shows that the *helper* machine is the part of two network families.
...
Having ssh access makes very easy for tunneling.Creating dynamically port forwarding leverages the SOCKS proxy.This will deliver all the traffic through ssh conncetion via given port to the destination server.Here we are forwarding all the traffics to the attacker machine.
...
The major concern is finding hosts.Having *nmap* would have been easier but mostly in real case scenario you won't find it.

![  no nmap ><](/assets/img/blog4-pivoting/14-nonmap.png)
<p align="center" style="color:white;">Fig:no nmap </p>

We are aware now that *helper* is the part of two networks.Both *helper* machine and *attacker* machine belong to same network **"172.16.100.0/24"**. Due to this reason we are now going to scan the network **"172.16.101.0/24"** to find all live hosts.

Using **ping sweep** technique to scan new hosts. The below one liner code will run in parallel mode.
...
We have now gather total 5 live hosts.
...
<span style="font-weight: bold; font-size: 2em;">Vcitim 1</span>

The reason why we cannot ping is because proxies do not support **icmp** protocols.Their working OSI layers are different.
...
As I said in the beginning of this blog there is a vunerability in this tool. Actually there is a **tar2slip** vulnerability.
...
```
**4.Chick on "New Host" then upload the tar file "pay.tar"**

**5.Check the for the file.**
...
<span style="font-weight: bold; font-size: 2em;">Vcitim 2</span>

Again we start with the same tedious *nmap* scan. There is a http service running on port 80.
...
For now our taget is http service.

Seems like it is running a wordpress cms.

![  curl victim2  ><](/assets/img/blog4-pivoting/26-victim2-wp-curl.png)
<p align="center" style="color:white;">Fig:Check</p>

If we have not automate the installation process with **wp-cli** then we should have first finished the wordpress installation process manually.
...
It is already configured and installed.The username and password for wp-admin is **"admin"** and **"adminpass"** respectively.
...
Now save the below poc.Check if you have set correct http_server and target.
...
So before executing the poc file we have to first ensure that everything is working properly. Getting reverse shell directly from **victim2** machine to **attacker** machine is not possible.Although we having running dynamic port forwarding it is still not going to work.

![ check  ><](/assets/img/blog4-pivoting/30-nonc.png)

<br>

**poc.sh** file downloads the **rev.sh** from the **attacker** machine to the **victim2** machine. But it is not making connection with the **http server**.
...
This will forward all the traffic (creating a tunnel) from remote port (**victim2**) 80 to local machine on port 80 (**attacker**).
Now when we run the **poc.sh** file, it can make connection with attacker python http server. Let's apply this.
